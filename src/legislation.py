...
class LegislationScraper(ScraperBase):
    """Scraper for comprehensive Acts/Bills extraction (Async)."""
    ...
    async def scrape(self, min_acts: int = 50) -> List[Dict]:
        """
        ...
        # Try new site first (currently commented out in original sync version too)
        # self.logger.info("Trying new site...")
        # acts = await self._scrape_new_site(min_acts)
        # if acts:
        ...
        acts = await self._scrape_old_site(min_acts)
    ...
    async def _scrape_new_site(self, min_acts: int) -> List[Dict]:
    ...
    async def _scrape_old_site(self, min_acts: int) -> List[Dict]:
    ...
    def _extract_act_data_new(self, element) -> Optional[Dict]:
        ...
        try:
            ...
            chapter_elem = element.find(string=re.compile(r'Cap\.?\s*\d+|Chapter\s*\d+', re.I))
            if chapter_elem:
            ...
            if pdf_elem:
                href = pdf_elem['href']
                act_data['download_url'] = urljoin(self.new_base_url, href) if not href.startswith(('http:', 'https:')) else href
            
            link_elem = element.find('a', href=True)
            if link_elem and not link_elem['href'].endswith('.pdf'):
                href = link_elem['href']
                act_data['source_url'] = urljoin(self.new_base_url, href) if not href.startswith(('http:', 'https:')) else href
    ...
    def _extract_act_data_table(self, row) -> Optional[Dict]:
        ...
        try:
            ...
            act_data['act_title'] = cols[0].get_text(strip=True)
    ...
    async def _save_to_elasticsearch(self, data: List[Dict]):
        ...
        try:
            ...
            for i, act in enumerate(data):
                ...
                tasks.append(self.es_config.index_document(act))
...
async def main():
    ...
    try:
        acts = await scraper.scrape(min_acts=50)
        if acts:
            await scraper.save_data(acts)
